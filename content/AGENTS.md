...
- Automatically suggest a Conventional Commit message only when the entire user-requested task is complete and the task
  changed repository files. Repository changes alone do not trigger this suggestion. If any planned phase or required
  work remains, omit the commit message from intermediate responses, even when the current phase changed files. After
  the final phase is complete, provide one message covering the entire task.
